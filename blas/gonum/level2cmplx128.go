		...
		if uplo == blas.Upper {
			...
			if incX == 1 {
				for i := range x[:n] {
					...
					if n-i-1 > 0 {
						x[i] += c128.DotuUnitary(ap[kk+1:kk+n-i], x[i+1:n])
					}
					kk += n - i
				}
			} else {
	...
	if trans == blas.Trans {
		...
		if uplo == blas.Upper {
			...
			if incX == 1 {
				for i := range x[:n] {
					if i > 0 {
						c128.AxpyUnitary(x[i], ap[i*lda:i*lda+i], x[:i])
					}
					if diag == blas.NonUnit {
						x[i] *= ap[i*lda+i]
					}
				...
				for i := 0; i < n; i++ {
					if i > 0 {
						c128.AxpyInc(x[ix], ap[i*lda:i*lda+i], x, uintptr(i), 1, uintptr(incX), 0, uintptr(kx))
					}
					if diag == blas.NonUnit {
						x[ix] *= ap[i*lda+i]
					}
	...
	if uplo == blas.Upper {
		...
		if incX == 1 {
			for i := n - 1; i >= 0; i-- {
				xi := x[i]
				if diag == blas.NonUnit {
					x[i] *= cmplx.Conj(ap[i*lda+i])
				}
				for j := i + 1; j < n; j++ {
					x[j] += xi * cmplx.Conj(ap[i*lda+j])
				}
			}
		} else {
			ix := kx + (n-1)*incX
			for i := n - 1; i >= 0; i-- {
				xi := x[ix]
				if diag == blas.NonUnit {
					x[ix] *= cmplx.Conj(ap[i*lda+i])
				}
				jx := ix + incX
				for j := i + 1; j < n; j++ {
					x[jx] += xi * cmplx.Conj(ap[i*lda+j])
					jx += incX
				}
				ix -= incX
			}
		...
		if incX == 1 {
			...
			for i := 0; i < n; i++ {
				jx := kx
				for j := 0; j < i; j++ {
					x[jx] += x[ix] * cmplx.Conj(ap[i*lda+j])
					jx += incX
				}
				if diag == blas.NonUnit {
					x[ix] *= cmplx.Conj(ap[i*lda+i])
